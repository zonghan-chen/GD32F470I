...
\subsection{10 DAC Output Voltage Value}

\begin{enumerate}
    \item Use the jumper cap JP23 to jumper to the DAC.
    \item LED 1, LED 2 and LED 3 flash three times for test.
    \item Use the voltmeter measure PA4 or DAC\_OUT0 on JP7, its value is $1.65V$.
\end{enumerate}

\textbf{DAC data configuration:}
\begin{table}[htbp]
    \centering
    \caption{DAC Data Configuration}
    \begin{tabular}{|c|c|}
        \hline
...
\textbf{Formula:}
\begin{itemize}
    \item \textbf{DAC output voltage ($V_{DAC}$) :}
          \begin{equation}
              V_{DAC}=\frac{DAC_{value}*V_{REFP}}{2^{bits}}
              \label{eq:DAC-output-voltage}
          \end{equation}
          \begin{table}[htbp]
              \centering
              \caption{Parameters in Equation \eqref{eq:DAC-output-voltage}}
              \begin{tabular}{|c|c|}
                  \hline
                  Parameter     & Explanation                               \\
                  \hline
